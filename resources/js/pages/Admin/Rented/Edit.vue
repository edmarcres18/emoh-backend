...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit Rental',
        href: `/admin/rented/${props.rented.id}/edit`,
    },
];
...
const form = useForm({
    client_id: props.rented.client_id.toString(),
    property_id: props.rented.property_id.toString(),
    monthly_rent: props.rented.monthly_rent,
    security_deposit: props.rented.security_deposit || '',
    start_date: formatDateForInput(props.rented.start_date),
    end_date: formatDateForInput(props.rented.end_date),
    status: props.rented.status,
    terms_conditions: props.rented.terms_conditions || '',
    notes: props.rented.notes || '',
    contract_signed_at: formatDateForInput(props.rented.contract_signed_at),
});

const isSubmitting = ref(false);
const selectedProperty = ref<Property | null>(
    props.properties.find(p => p.id === props.rented.property_id) || null
);
...
watch(() => form.property_id, async (propertyId, oldPropertyId) => {
    if (propertyId && propertyId !== oldPropertyId) {
        selectedProperty.value = props.properties.find(p => p.id.toString() === propertyId) || null;
        
        // Only fetch from API if property actually changed
        if (propertyId !== props.rented.property_id.toString()) {
            try {
...
const submit = () => {
    ...
    form.put(`/admin/rented/${props.rented.id}`, {
        onSuccess: () => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Rental</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Update rental record for {{ rented.client.name }} - {{ rented.property.property_name }}
                    </p>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div v-else-if="rateAutoFilled && form.monthly_rent" class="mt-2 flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
                                ...
                                Monthly rent automatically updated to ₱{{ formatCurrency(form.monthly_rent) }} from property's estimated rate
                            </div>
                        </div>
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-blue-900 dark:text-blue-100 mb-1">Rental Update Guidelines</h3>
                        <ul class="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                            <li>• Changing property may affect availability if current property is active</li>
                            <li>• Monthly rent will auto-update when changing properties</li>
                            <li>• Status changes may affect property availability</li>
                            <li>• Contract signed date should reflect actual signature date</li>
                            <li>• Use "Active" status only when contract is fully executed</li>
                        </ul>
                    </div>
                </div>
            </div>
