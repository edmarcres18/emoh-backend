...
const validSortOptions = ['latest', 'oldest', 'name_asc', 'name_desc', 'code_asc', 'code_desc'];
...
const debouncedSearch = debounce((query: string) => {
    router.get('/locations', { search: query, sort: sortBy.value, per_page: perPage.value }, {
        preserveState: true,
        replace: true,
    });
}, 300);
...
watch(sortBy, (newSort) => {
    ...
    router.get('/locations', { search: searchQuery.value, sort: validSort, per_page: perPage.value }, {
        preserveState: true,
        replace: true,
    });
});
...
watch(perPage, () => {
    router.get('/locations', { search: searchQuery.value, sort: sortBy.value, per_page: perPage.value }, {
        preserveState: true,
        replace: true,
    });
});
...
const confirmDelete = () => {
    if (locationToDelete.value && !isDeleting.value) {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Locations</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage and organize your location data</p>
                </div>
                <button 
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-900 px-3 py-2 rounded-lg shadow-sm">
                        <span class="font-medium">{{ locations.from || 0 }}</span>-<span class="font-medium">{{ locations.to || 0 }}</span> of <span class="font-medium">{{ locations.total }}</span> locations
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="location in locations.data" :key="location.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-8 w-8 bg-indigo-100 dark:bg-indigo-900/30 rounded-full flex items-center justify-center">
                                            <span class="text-xs font-medium text-indigo-600 dark:text-indigo-400">{{ location.id }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-600 dark:text-gray-300 max-w-xs truncate" :title="location.description">
                                        {{ location.description || 'No description' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatDate(location.created_at) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button 
                                            v-if="canViewLocations"
                                            @click="router.get(`/locations/${location.id}`)"
                ...
                <!-- Pagination -->
                <div v-if="locations.last_page > 1" class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
        ...
        <DeleteModal
            ...
            :message="`Are you sure you want to delete '${locationToDelete?.name}' (${locationToDelete?.code})? This action cannot be undone and will permanently remove this location from your system.`"
