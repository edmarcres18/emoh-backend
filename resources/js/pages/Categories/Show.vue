...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-2">
                        <div class="flex-shrink-0 h-12 w-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-xl flex items-center justify-center">
                            <span class="text-lg font-bold text-indigo-600 dark:text-indigo-400">{{ category.id }}</span>
                        </div>
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ category.name }}</h1>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Category Details</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Category Information</h2>
                        </div>
                        <div class="p-6 space-y-6">
                            <!-- Name -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Category Name</label>
                                <div class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ category.name }}</div>
                            </div>

                            <!-- Description -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Description</label>
                                <div v-if="category.description" class="text-gray-700 dark:text-gray-300 leading-relaxed">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ formatDateRelative(category.created_at) }}
                                    </div>
                                </div>
                                <div v-if="category.updated_at !== category.created_at">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ formatDateRelative(category.updated_at) }}
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Category Stats</h3>
                        </div>
                        <div class="p-6 space-y-4">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Category ID</span>
                                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">#{{ category.id }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Name Length</span>
                                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ category.name.length }} characters</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Description</span>
                                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                    {{ category.description ? `${category.description.length} characters` : 'Not set' }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between">
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <h4 class="text-sm font-semibold text-blue-900 dark:text-blue-100 mb-1">Category Management</h4>
                                <ul class="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                                    <li>• Use "Edit" to modify category details</li>
                                    <li>• Deleting a category is permanent</li>
                                    <li>• Consider the impact on existing content</li>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        <DeleteModal
            ...
            :message="`Are you sure you want to delete '${category.name}'? This action cannot be undone and will permanently remove this category from your system.`"
